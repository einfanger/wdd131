...
function createStars(rating) {
  const full = '★'.repeat(Math.max(0, Math.floor(rating)));
...
function buildCardHTML(recipe) {
  ...
  const imgHTML = recipe.image
    ? `<img src="${escapeHtml(recipe.image)}" alt="${escapeHtml(recipe.title)}" onerror="this.style.display='none'; this.nextElementSibling && (this.nextElementSibling.style.display='flex')">`
    : `<div class="img-placeholder">No Image</div>`;

  const placeholderHTML = `<div class="img-placeholder" style="display:none">No Image</div>`;

  const stars = createStars(recipe.rating);
  ...
  return `
    <div class="recipe-card" data-id="${recipe.id}">
      ...
      <div class="recipe-info">
        <div class="tags-line">${tagsHTML}</div>
        <h2 class="recipe-title">${escapeHtml(recipe.title)}</h2>
        <span class="rating" aria-label="Rating">${escapeHtml(stars)}</span>
...
function renderRandomRecipe() {
  ...
  randomContainer.innerHTML = `
    <h3 style="margin-top:0">Random recipe</h3>
    ${buildCardHTML(random)}
  `;
}
...
function renderResults(list) {
  ...
  if (!list || list.length === 0) {
    resultsContainer.innerHTML = `<p>No recipes found. Try a broader search.</p>`;
    return;
  }

  const sorted = list.slice().sort((a, b) => a.title.localeCompare(b.title, undefined, { sensitivity: 'base' }));
  resultsContainer.innerHTML = sorted.map(r => buildCardHTML(r)).join('');
}
...
function doSearch() {
  ...
  if (randomContainer) randomContainer.style.display = query ? 'none' : 'block';
  ...
  const filtered = recipes.filter(r => matchesRecipe(r, keywords));
